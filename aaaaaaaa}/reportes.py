...
# =========================================================================
# FUNCIÓN PRINCIPAL DE REPORTES (SÚPER SIMPLE)
# =========================================================================
def reporte_detallado(fecha_inicio, fecha_fin, categoria_opcional=None):
    ...
    #Bucle para revisar uno por uno todos los gastos cargados
    for g in todos:
    ...
    #Bucle para calcular los totales usando la lista ya filtrada
    for g in filtrados:
        #Extrae el monto del gasto actual y lo convierte a número decimal
        monto = float(g["monto"])
        #Suma el monto al contador del total general
        total_general += monto
        
        #Obtiene la categoría del gasto actual en minúsculas
    ...
    #Crea la lista de gastos limpia con la estructura exacta que pide el examen
    lista_limpia = []
    #Bucle para formatear los gastos filtrados
    for g in filtrados:
        #Agrega al arreglo el diccionario con las 4 llaves solicitadas
        lista_limpia.append({
            "fecha": g["fecha"],
    ...
    #Construye la estructura final del reporte JSON requerido
    reporte_json = {
